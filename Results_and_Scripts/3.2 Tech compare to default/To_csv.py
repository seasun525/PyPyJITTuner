...
def get_performance_data(benchmark, last_iterations):
	...
	result_list = []
	for config_index in [0,1,3,5,8,11]:#range(5):
		print [0,1,3,5,8,11].index(config_index)
		temp_list = []
		for run_number in range(30):
			for j in range(50 - int(warmup_iter_list[0])):
...
def save_as_csv(benchmark, result_list):
	for config_index in range(6):
		file_out = open(benchmark+'_'+str(5+config_index)+'_response_time.csv', 'w')
...
def main():
	benchmarks = ['plaintext', 'update']#, 'query']#, 'rietveld', 'html5lib', 'pidigits']#'bm_icbd' , 'twisted_web'
	warmup_iterations = [34,29,10,11,42,11,0]
	for benchmark in benchmarks:
		data_list = json.load(open(benchmark+'_response.json','r'))
