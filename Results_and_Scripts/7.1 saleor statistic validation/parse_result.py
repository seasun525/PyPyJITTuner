...
class result_parser():
    ...
    def parse_for_pages(self, input_folder, output_name):
        result = []
        response_time_list_for_page = self.parse_response_1(input_folder)
        for i in range(len(self.page_index)):
            #result.append(sum(response_time_list_for_page[i])/len(response_time_list_for_page[i]))
            result.append(response_time_list_for_page[i])
        #CPU_list, Memory_list = self.parse_pids(input_folder, 'pid')
        #result.append(sum(CPU_list)/len(CPU_list))
        #result.append(sum(Memory_list)/len(Memory_list))
        ...
        output_file = open(input_folder+'/'+output_name, 'w')
...
if __name__ == '__main__':
    ...
    for i in range(3):
        start_time = 20*60*i
        end_time = 20*60*(i+1)
        Parser = result_parser(start_time, end_time)
        Parser.parse_for_pages(input_folder, 'div_individual_value'+str(i))
    #input_folder = sys.argv[1]
