...
individual_list <- c('div_individual_value0','div_individual_value1','div_individual_value2')

root_dir <- '111010110011111011/'
ind_num <- 3
obj_num <- 12

get_data <- function(individual, flag){
  
  benchmark_file <- paste(root_dir,individual, sep='')
...
for(i in 1:obj_num){
  ...
  print(i)
  write.csv(temp_matrix, file=paste(root_dir, 'effect_size_matrix_', i,'.csv'))
  print('+++++++++++++++++++++++++')
}

temp_matrix <- matrix(nrow = obj_num*2+1, ncol = 8)
for(i in 1:obj_num){
  temp_vector <- c()
  for(j in 2:ind_num){
  ...
  temp_matrix[obj_num+1, ]<-c(0,0,0,0,0,0,0,0)
...
print(i)
write.csv(temp_matrix, file=paste(root_dir, 'effect_size_for_slots.csv'))
print('+++++++++++++++++++++++++')
