...
label_upper <- c(0.0315, 0.025, 0.0070, 0.0292, 0.561, 2.68, 6.5)#2.65
...
for(index in 1:length(benchmarks)){
  ...
  text_jitting <- textGrob("jitting", hjust=0, gp=gpar(fontfamily ="serif", fontsize=20))
  ...
  myplot1 <- ggplot(df) +
    ...
    annotation_custom(block1,xmin=2,xmax=2.4,ymin=label_up,ymax=label_up) +
    annotation_custom(text_jitting,xmin=warmup/2-1,xmax=warmup/2-1,ymin=label_up,ymax=label_up) +
    annotation_custom(block2,xmin=warmup+5.35,xmax=warmup+5.35,ymin=label_up,ymax=label_up) + #0.73
    annotation_custom(text_jitted,xmin=(warmup+50)/2,xmax=(warmup+50)/2,ymin=label_up,ymax=label_up) +
    annotation_custom(block3,xmin=42.8,xmax=43.7,ymin=label_up,ymax=label_up) +
    theme(plot.margin = unit(c(2,1,1,1), "lines"), text = element_text(family="serif", size=20))
  
  myplot2 <- ggplot(df, height = 200) +
    geom_line(aes(x=iteration, y = jitline, size = 0.3)) +
    geom_line(aes(x=factor(iteration), y = jitline, size = 0.3)) +
    scale_size(range=c(0.1, 1), guide=FALSE) +
    
    #geom_boxplot(aes(x=factor(iteration), y=response)) +
    #scale_y_continuous(limits = c(1, upper), sec.axis = sec_axis(~(.-1)*90/1.5+450, name = "Number of jitted lines")) +
    scale_x_discrete(breaks = seq(0, 50, 5)) +
    ylab("Number of jitted lines") +
    xlab("Number of iterations")+
    geom_vline(xintercept = warmup, color = "red", linetype = "dashed") +
    theme(plot.margin = unit(c(0,1,3,1), "lines"), text = element_text(family="serif", size=20))
  
  #change the font of jitting and jitted to be same as x-axis.
  #print(myplot)
  #myplot <- grid.arrange(myplot1, myplot2, nrow =2)
  
  gt1 <- ggplot_gtable(ggplot_build(myplot1))
  gt1$layout$clip[gt1$layout$name == "panel"] <- "off"
  
  gt <- grid.arrange(gt1, myplot2, nrow =2)
  myplot<-grid.draw(gt)
