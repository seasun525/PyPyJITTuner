...
string_list <- c('mongo_statistic_CPU.csv', 'mongo_statistic_Memory.csv', 'worker_statistic_CPU.csv', 'worker_statistic_Memory.csv')
string_list2 <- c('13_mongo_CPU', '14_mongo_Memory', '15_worker_CPU', '16_worker_Memory')
get_data <- function(individual, flag){

  benchmark_file <- paste('tops',individual,'\\', flag, sep='')
...
for(i in 1:4){
  ...
  for(j in 1:4){
    ...
    temp_list[[j]] <- get_data(individual_list[[i]], string_list[[j]])[[1]]
    print(temp_list[[j]])
  }
